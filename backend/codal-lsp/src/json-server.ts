...
export class JsonServer {
    ...
    protected hover(
        ...
        const jsonDocument = this.getJSONDocument(document);
        return this.jsonService.doHover(
            document,
            params.position,
            jsonDocument
        );
    ...
    protected async resolveSchema(url: string): Promise<string> {
        ...
        try {
            const response = await requestLight.xhr({
                url,
                followRedirects: 5,
            });
            ...
            return Promise.reject(
                err.responseText ||
                    requestLight.getErrorStatusDescription(
    ...
    protected completion(
        ...
        const jsonDocument = this.getJSONDocument(document);
        return this.jsonService.doComplete(
            document,
            params.position,
            jsonDocument
        );
